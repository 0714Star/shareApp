...
const uni_modules_uniMediaLibrary_common_uploader_provider_base = require("./base.js");
const uniMediaLibraryCo = common_vendor.Zs.importObject("uni-media-library-co", {
  customUI: true
});
class InternalUploadProvider extends uni_modules_uniMediaLibrary_common_uploader_provider_base.UploadProvider {
  async upload(files) {
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const ext = await file.getFileType();
      try {
        const result = await common_vendor.Zs.uploadFile({
...
exports.InternalUploadProvider = InternalUploadProvider;
